...
public class PwdEncryptor {
    ...
    public static final String PASSWORDS_ENCRYPTION_ALGORITHM = "SSHA";
    ...
    /*protected*/public static String encodePassword(
        ...
        try {
            if (algorithm.equals(TYPE_BCRYPT)) {
                ...
                byte[] clearTextPasswordBytes =
                        clearTextPassword.getBytes("UTF8");//Digester.ENCODING);
    ...
    private static byte[] _getSaltFromBCrypt(String bcryptString)
        ...
        try {
            ...
            if (StringUtils.isEmpty(bcryptString)) {
                String salt = null; // BCrypt.gensalt();
    ...
    private static byte[] _getSaltFromCrypt(String cryptString)
        ...
        try {
            ...
            if (StringUtils.isEmpty(cryptString)) {
    ...
    /*private*/public static byte[] _getSaltFromSSHA(String sshaString)
            throws PwdEncryptorException {

        byte[] saltBytes = new byte[8];

        //if (Validator.isNull(sshaString)) {
        if (StringUtils.isEmpty(sshaString)) {
            ...
            try {
                byte[] digestPlusSalt = ByteArray.fromBase64String(sshaString);
